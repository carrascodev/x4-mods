...
SectorMatchManager::SectorMatchManager()
    : X4ScriptSingleton("SectorMatchManager")
    , m_localPlayerId("")
    , m_currentSector("")
    , m_interpolationDelayMs(100.0f)
    , m_maxSnapshotAgeMs(1000)
    , m_cleanupIntervalMs(5000)
...
bool SectorMatchManager::Initialize(const std::string& localPlayerId)
{
    ...
    SetInitialized(true);
    LogInfo("SectorMatchManager initialized for player: %s", localPlayerId.c_str());
    return true;
}
...
void SectorMatchManager::ChangeSector(const std::string& newSector)
{
    ...
    LogInfo("Changing sector from '%s' to '%s'", m_currentSector.c_str(), newSector.c_str());
    ...
    if (rtClient && rtClient->IsConnected())
    {
        if (rtClient->JoinOrCreateMatch(newSector))
        {
            ...
            SendLocalPosition(localShip.position, localShip.rotation, localShip.velocity);
            LogInfo("Joined match for sector %s and sent local player data", newSector.c_str());
        }
    }
}

void SectorMatchManager::OnSectorJoined(const std::string& sector, const PlayerShip& playerShip)
{
    ...
    LogInfo("Player %s joined sector %s", playerShip.player_id.c_str(), sector.c_str());
}
...
void SectorMatchManager::UpdateRemotePlayer(const std::string& playerId,
...
std::vector<float> SectorMatchManager::GetInterpolatedPosition(const std::string& playerId) const
{
    auto it = m_playerShips.find(playerId);
    if (it != m_playerShips.end())
    {
...
void SectorMatchManager::RemovePlayer(const std::string& playerId)
{
    ...
    if (it != m_playerShips.end())
    {
        LogInfo("Removing player %s from sector %s", playerId.c_str(), m_currentSector.c_str());
...
void SectorMatchManager::Update(float deltaTime)
{
    ...
    if (now - m_lastCleanupTime > std::chrono::milliseconds(m_cleanupIntervalMs))
    {
        CleanupStalePlayers();
