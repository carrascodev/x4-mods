...
string GetLogDir() {
    if (!g_module_dir.empty()) return g_module_dir;

    // Use Documents folder for logs - more reliable than mod directory
    char documentsPath[MAX_PATH];
    if (SHGetFolderPathA(NULL, CSIDL_PERSONAL, NULL, 0, documentsPath) == S_OK) {
        g_module_dir = string(documentsPath) + "\\Egosoft\\X4\\logs\\Nakama";
...
string GetLogTimestamp() {
    if (!g_log_timestamp.empty()) return g_log_timestamp;
...
void AppendToModLog(const string& s) {
    ...
    string logfile = GetLogDir() + "\\nakama_mod" + GetLogTimestamp() + ".log";
    
    // Use Windows API for file I/O
    HANDLE hFile = CreateFileA(logfile.c_str(), FILE_APPEND_DATA, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
